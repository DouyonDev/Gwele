...
class _DetailEquipeState extends State<DetailEquipe> {
  ...
  Future<void> getLeadSecond() async {
    this.leader = await UtilisateurService().utilisateurParId(widget.equipeInfo.leaderId);
    this.second = await UtilisateurService().utilisateurParId(widget.equipeInfo.secondId!);
  }
    ...
    return Column(
      ...
      children: [
        ...
        ListTile(
          ...
          radius: 40.0,
          backgroundImage: leader?.imageUrl != null
          ? NetworkImage(leader!.imageUrl)  // Utiliser NetworkImage pour les images depuis Firebase Storage
        ...
        if(second != null)
          ListTile(
            leading: CircleAvatar(
              radius: 40.0,
              backgroundImage: second?.imageUrl != null
                  ? NetworkImage(second!.imageUrl)  // Utiliser NetworkImage pour les images depuis Firebase Storage
                  : const AssetImage('assets/images/boy.png') as ImageProvider,
            ),
            title: Text(
              ...
              return Column(
                children: snapshot.data!.map((membre) {
                  return ListTile(
                    leading: CircleAvatar(
                      radius: 40.0,
                      backgroundImage: membre.imageUrl != null
                          ? NetworkImage(membre.imageUrl)  // Utiliser NetworkImage pour les images depuis Firebase Storage
                          : const AssetImage('assets/images/boy.png') as ImageProvider,
                    ),
                    title: Text(
